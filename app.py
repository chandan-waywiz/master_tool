...
GEMINI_MODEL_NAME = 'gemini-2.5-flash' # Needed for image analysis
...
def create_itinerary_document(data, traveler_info):
    ...
    # p1.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY  # Removed justification
    ...
    # p2.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY  # Removed justification
    ...
    # p3.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY  # Removed justification
    ...
    run_docs = p_docs.add_run("Enclosed are the supporting documents for my visa application:")
    run_docs.bold = True
    set_spacing(p_docs, Pt(6))
    for item in [
...
def create_cover_letter_document(data, traveler_info):
    ...
    doc = Document()
    style = doc.styles['Normal']; font = style.font; font.name = 'NTR'; font.size = Pt(11)

    for i in range(1, 7):
        if f'Heading {i}' in doc.styles:
            style = doc.styles[f'Heading {i}']; font = style.font; font.name = 'NTR'; font.size = Pt(18); font.color.rgb = RGBColor(0,0,0)
    ...
    p_sincere = doc.add_paragraph("I sincerely appreciate your time and consideration of my application. I am fully committed to complying with all visa regulations and will adhere to the laws and values of "); p_sincere.add_run(formatted_countries).bold = True; p_sincere.add_run(" throughout my visit."); set_spacing(p_sincere, Pt(12))

    p_contact = doc.add_paragraph("If any additional documentation or information is required, please feel free to contact me at "); p_contact.add_run(traveler_info['phone']).bold = True; p_contact.add_run(" or "); p_contact.add_run(traveler_info['email']).bold = True; p_contact.add_run(".Thank you for your time and consideration. I look forward to a positive response and the opportunity to experience the beauty of "); p_contact.add_run(formatted_countries).bold = True; p_contact.add_run("."); set_spacing(p_contact, Pt(12))
...
@app.route('/preview_id_scan', methods=['POST'])
def preview_id_scan():
    print("\n--- New ID Preview Request ---")
    file_path = None
    try:
        ...
        if not allowed_file(safe_filename):
            return jsonify({"error": "Invalid file format."}), 415

        file_path = os.path.join(app.config['UPLOAD_FOLDER'], safe_filename)
        file.save(file_path)

        # --- Image Processing Logic (No Database Interaction) ---
        full_ocr_text = extract_text_paddle(file_path, lang='en')
        if not full_ocr_text or not full_ocr_text.strip():
            return jsonify({"error": "Could not find any text in the image."}), 400

        doc_type = detect_document_type(full_ocr_text)
        if doc_type == "unknown":
            return jsonify({"error": "Could not identify the document type."}), 400

        redacted_text = full_ocr_text
        found_id = None
        id_field_name = f"{doc_type.capitalize()}_No"
        ...
        if genai and API_KEY:
            ...
            if gemini_output:
                try:
                    clean_response = gemini_output.strip().removeprefix("```json").removesuffix("```").strip()
                    gemini_parsed_data = json.loads(clean_response)
                except json.JSONDecodeError:
                    return jsonify({"error": "Failed to parse analysis data."}), 500
        
        final_extracted_data = gemini_parsed_data.copy()
        if found_id:
            final_extracted_data[id_field_name] = found_id
        
        # Pass along metadata needed for the save step
        final_extracted_data['_doc_type'] = doc_type
        final_extracted_data['_original_filename'] = safe_filename

        return jsonify(final_extracted_data)

    except Exception as e:
        print(f"ERROR in /preview_id_scan: {e}")
        return jsonify({"error": "An unexpected server error occurred during preview."}), 500
    finally:
        if file_path and os.path.exists(file_path):
            os.remove(file_path)

@app.route('/confirm_and_save_data', methods=['POST'])
def confirm_and_save_data():
    ...
    try:
        ...
        # Extract metadata and remove it from the main data object
        doc_type = data_to_save.pop('_doc_type', 'unknown')
        original_filename = data_to_save.pop('_original_filename', 'unknown_file')
        
        user_id = None
        db_user_status = "Skipped (Insufficient data)"
        db_document_status = "Skipped"

        computed_name, computed_dob_yob = compute_unique_identifiers(data_to_save, doc_type)
        

        if users_collection is not None and computed_name and computed_dob_yob:
            title_case_name = computed_name.title()
            user_filter = {"name": title_case_name, "dob_yob": computed_dob_yob}
            # Get phone, email, passport number from data_to_save
            phone = data_to_save.get('phone')
            email = data_to_save.get('email')
            passport_number = data_to_save.get('Passport_No') or data_to_save.get('passport_number')
            user_update = {
                "$setOnInsert": {
            ...
            if phone:
                user_update["$set"]["phone"] = phone
            if email:
                user_update["$set"]["email"] = email
            if passport_number:
                user_update["$set"]["passport_number"] = passport_number

            user_record = users_collection.find_one_and_update(
                user_filter, user_update, upsert=True, return_document=ReturnDocument.AFTER
            )
            user_id = user_record['_id']
            db_user_status = "Success (Found/Created)"

        if user_id and docs_collection is not None:
            existing_doc = docs_collection.find_one({"user_id": user_id, "doc_type": doc_type})
            if existing_doc:
                db_document_status = "Skipped (Already Exists)"
            else:
                sanitized_data = sanitize_mongodb_keys(data_to_save)
                document_record = {
                    "user_id": user_id,
                    "doc_type": doc_type,
                ...
                docs_collection.insert_one(document_record)
                db_document_status = "Success"

        return jsonify({
            "db_user_status": db_user_status,
            "db_document_status": db_document_status
        })

    except Exception as e:
        print(f"ERROR in /confirm_and_save_data: {e}")
        return jsonify({"error": "An unexpected server error occurred during save."}), 500
...
def generate_document_api():
    try:
        ...
        # --- NEW: UPSERT USER LOGIC STARTS HERE ---
        passport_number = user_data.get('passport_number')
        full_name = user_data.get('name')
        phone = user_data.get('phone') if 'phone' in user_data and user_data.get('phone') is not None else ''
        email = user_data.get('email') if 'email' in user_data and user_data.get('email') is not None else ''

        if passport_number and users_collection is not None:
            print(f"Upserting user details for passport: {passport_number}")
            print("Payload:", user_data)
            user_filter = {'passport_number': passport_number}
            user_update = {
                '$set': {
                    'name': full_name,
                    'phone': phone,
            ...
            print("Upsert filter:", user_filter)
            print("Upsert update:", user_update)
            try:
                ...
                print(f"Successfully upserted user: {full_name}")
            except Exception as db_error:
                print(f"DATABASE ERROR during user upsert: {db_error}")
                # For now, we'll just log it and continue generating the document
        # --- NEW: UPSERT USER LOGIC ENDS HERE ---
        ...
        if file_ext in ['.png', '.jpg', '.jpeg']:
            ...
            if users_collection is not None and title_case_name and computed_dob_yob:
                try:
                    ...
                    # Use found_id from OCR extraction if available, fallback to Gemini if not
                    passport_number = found_id if found_id else (gemini_parsed_data.get('passport_number') if gemini_parsed_data else None)
                    user_filter = {"name": title_case_name, "dob_yob": computed_dob_yob}
                    user_update = {
                        "$setOnInsert": {
                            "name": title_case_name,
